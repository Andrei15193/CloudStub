...
namespace CloudStub.AzureDataTables.Tests.Table.Transactions.Async
{
    public class TableClientTransactionUpdateReplaceEntityTests : BaseTableCloudStubTests
    {
        ...
        public async Task SubmitTransactionAsync_WhenETagsMatch_ReplacesEntity()
        {
            var guid = Guid.NewGuid();
            await TableClient.CreateAsync();
            var addResult = await TableClient.AddEntityAsync(new TestEntity
            {
                PartitionKey = "partition-key",
                RowKey = "row-key",
                Int32Prop = 14,
        ...
        public async Task SubmitTransactionAsync_WhenETagsMismatch_ThrowsException()
        {
            ...
            await TableClient.UpdateEntityAsync(new TableEntity { PartitionKey = "partition-key", RowKey = "row-key" }, addResult.Headers.ETag.Value, TableUpdateMode.Replace);
